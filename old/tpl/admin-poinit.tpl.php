...
<div class="wrap loco-admin loco-init"><?php 
    ...
    <h2><?php 
        Loco::h( __('Initialize new translations in %s','loco-legacy'), $domain )?> 
    </h2>
    ...
    <form action="" method="get" class="wp-core-ui" id="loco-msginit">
        ...
        <p>
            <select name="common-locale">
                <option value="">
                    <?php Loco::h( __('Select from common languages','loco-legacy') )?> 
                </option><?php
        ...
        <p>
            <label for="f-loco-locale">
                &ndash;
                <?php Loco::h( _x('or enter any language code','Form label','loco-legacy') )?>:
                <br />
            </label>
        ...
        <p>
            <label>
                ...
                <?php echo sprintf( _x('create in <code>%s</code>','Form label','loco-legacy' ), LocoAdmin::trim_path($pdir) )?> 
            </label>
            <br />
            <label>
                <input type="radio" name="gforce" value="1"<?php print( $is_global ? ' checked' : '') ?><?php print( $gdir_ok ? '' : ' disabled')?> />
                <?php Loco::h( _x('create in global languages directory','Form label','loco-legacy') )?> 
        ...
        <p class="submit">
            <input type="submit" value="<?php Loco::h( _x('Start translating','Submit button','loco-legacy') )?>" class="button button-primary button-large" disabled />
        </p>
    </form>
