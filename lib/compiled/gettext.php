...
class LocoHeaders extends ArrayIterator implements LocoArrayInterface {
    ...
    public function keys() {
        trigger_error( 'Is this required?', E_USER_NOTICE );
...
class LocoPoHeaders extends LocoHeaders {
    ...
    public function getCharset() {
        ...
        if ( is_null( $cs ) ) {
            ...
            if ( $raw && preg_match( '!\\bcharset[= ]+([-0-9a-z]+)!i', $raw, $r ) ) {
                $cs      = $r[1];
                $aliases = @mb_encoding_aliases( $cs );
                if ( false === $aliases ) {
                    $cs = 'UTF-8';
                } else if ( preg_grep( '/^(UTF-?8|US-?ASCII)/i', $aliases ) ) {
                    $cs = 'UTF-8';
                }
            }
            $this->cs = $cs;
    ...
    public static function fromMsgstr( $str ) {
        ...
        foreach ( preg_split( '/[\\r\\n]+/', $str ) as $line ) {
            ...
            if ( is_int( $i ) ) {
                ...
                $val = ltrim( substr( $line, ++ $i ), " \t" );
            } else {
                $val = $headers->offsetGet( $key ) . "\n" . $line;
            }
            if ( '' !== $key ) {
                $headers->offsetSet( $key, $val );
            }
        }
        $cs = $headers->getCharset();
        if ( $cs && 'UTF-8' !== $cs && 'UTF-8' !== mb_detect_encoding( $str, array( 'UTF-8', $cs ), true ) ) {
            foreach ( $headers as $key => $val ) {
                $headers[ $key ] = mb_convert_encoding( $val, 'UTF-8', array( $cs ) );
            }
        }
...
abstract class LocoGettextParser {
    ...
    protected function setHeader( LocoPoHeaders $head ) {
        $this->head = $head;
        if ( $cs = $head->getCharset() ) {
            if ( is_null( $this->cs ) ) {
                $this->setCharset( $cs );
            } else if ( $cs !== $this->cs ) {
                trigger_error( 'TODO: convert header from ' . $cs . ' to ' . $this->cs . ' ?', E_USER_NOTICE );
            }
        }
    ...
    protected function str( $str ) {
        if ( $this->cs ) {
            ...
            $enc      = mb_detect_encoding( $str, array( 'UTF-8', 'ISO-8859-1' ), true );
            $this->cs = $enc;
        }
        if ( 'UTF-8' !== $enc ) {
            $str = mb_convert_encoding( $str, 'UTF-8', array( $enc ) );
        }
...
class LocoPoParser extends LocoGettextParser implements Iterator {
    ...
    public function __construct( $src ) {
        if ( '' !== $src ) {
            ...
            if ( 'UTF-8' === mb_detect_encoding( $src, array( 'UTF-8', 'ISO-8859-1' ), true ) ) {
    ...
    public function next() {
        $valid = false;
        $entry = array( '#' => array(), 'id' => array( null ), 'str' => array( null ) );
        $i     = $this->i;
    ...
    public function parse( $limit = - 1 ) {
        ...
        $cs = $this->getCharset() or $this->setCharset( $cs = 'UTF-8' );
            ...
            if ( isset( $entry['id'][1] ) ) {
                ...
                while ( ++ $idx < $num ) {
                    ...
                    if ( 1 === $idx ) {
                        ...
                        if ( $lk && is_array( $prev_entry ) && isset( $prev_entry[1] ) ) {
    ...
    public function parse( $limit = - 1 ) {
        ...
        while ( $soffset < $targetOffset ) {
            ...
            if ( isset( $sources[1] ) ) {
                ...
                for ( $i = 1; $i < $nforms; $i ++ ) {
                    $r[] = array(
                        'source' => isset( $sources[ $i ] ) ? $this->str( $sources[ $i ] ) : sprintf( '%s (plural %u)', $r[ $p ]['source'], $i ),
                        'target' => isset( $targets[ $i ] ) ? $this->str( $targets[ $i ] ) : '',
                        'parent' => $p,
                        'plural' => $i,
                    );
                }
...
abstract class LocoPo {
    public static function pair( $key, $text, $width = 79, $eol = "\n", $esc = '\\n' ) {
        ...
        if ( $esc ) {
            ...
            $eol  = "\n";
            $text = preg_replace_callback( '/(?:\\r\\n?|\\n)/', array( __CLASS__, 'replace_br' ), $text, - 1, $nbr );
        }
        if ( $nbr ) {
    ...
    public static function refs( $text, $width = 76, $eol = "\n" ) {
        ...
        if ( $width ) {
            $text = wordwrap( $text, $width, $eol . '#: ', false );
        }
    ...
    public static function prefix( $text, $prefix, $eol = "\n" ) {
        ...
        return $prefix . implode( $eol . $prefix, $lines );
    }
}
...
class LocoPoIterator implements Iterator, Countable {
    ...
    public function current() {
        ...
        while ( isset( $po[ ++ $i ]['parent'] ) ) {
            $this->i   = $i;
            $plurals[] = new LocoPoMessage( $po[ $i ] );
        }
        if ( $plurals ) {
    ...
    public function getHeaders() {
        if ( ! $this->headers ) {
            $header = $this->po[0];
            if ( 0 === $this->z ) {
                $this->headers = LocoPoHeaders::fromMsgstr( $header['target'] );
            } else {
    ...
    public function sort( $func = null ) {
        ...
        foreach ( $order as $msg ) {
            $po[] = $msg->getArrayCopy();
            if ( is_array( $plurals = $msg->plurals ) ) {
                $index = count( $po ) - 1;
                unset( $po[ $index ]['plurals'] );
...
class LocoPoMessage extends ArrayObject {
    ...
    private function _getFlags() {
        ...
        if ( $f = $this->__get( 'format' ) ) {
            $flags[] = $f . '-format';
        } else if ( isset( $plurals[0] ) && ( $f = $plurals[0]->__get( 'format' ) ) ) {
            $flags[] = $f . '-format';
        }
    ...
    public function render( $width, $ref_width ) {
        $s = '';
        try {
            ...
            if ( is_array( $plurals = $this->__get( 'plurals' ) ) ) {
                if ( $plurals ) {
                    foreach ( $plurals as $i => $p ) {
                        if ( 0 === $i ) {
                            $s .= LocoPo::pair( 'msgid_plural', $p['key'], $width ) . "\n";
...
class LocoMo {
    ...
    public function __construct( Iterator $export, LocoHeaders $head = null ) {
        if ( $head ) {
            $this->head = $head;
        } else {
            $this->head = new LocoHeaders( array( 'Project-Id-Version' => 'Loco' ) );
        }
        $this->msgs = $export;
        $this->bin  = '';
    }
...
class LocoTokenizer implements LocoTokensInterface {
    ...
    public function define( $grep, $t = 0 ) {
        ...
        $sniff = substr( $grep, 2, 1 );
        if ( $sniff === preg_quote( $sniff, $grep{0} ) ) {
            $this->rules[ $sniff ][] = array( $grep, $t );
        } else {
            $this->rules[''][] = array( $grep, $t );
        }
    ...
    public function next() {
        ...
        while ( $offset <= $this->max ) {
            ...
            if ( is_null( $t ) && preg_match( '/^./u', $text, $match ) ) {
                $s = $match[0];
                $t = self::T_UNKNOWN;
            }
            $length = strlen( $s );
            if ( 0 === $length ) {
                throw new Loco_error_ParseException( 'Failed to match anything at [' . $offset . ']' );
            }
            $offset += $length;
            ...
            $tok    = self::T_LITERAL === $t ? $s : array( $t, $s, $line, $column );
            $line   = $next_line;
            $column = $next_column;
            $this->len ++;
            break;
...
class LocoEscapeParser {
    ...
    public function unescape( $s ) {
        if ( '' !== $s ) {
            return $this->stripSlashes( preg_replace_callback( $this->grep, array( $this, 'unescapeMatch' ), $s ) );
        }

        return '';
    }

    public function unescapeMatch( array $r ) {
        $s = $r[0];
        $c = substr( $s, 1, 1 );
        if ( isset( $this->map[ $c ] ) ) {
            return $this->map[ $c ];
...
class LocoJsTokens extends LocoTokenizer {
    ...
    public static function decapse( $encapsed ) {
        ...
        $l = self::$lex;
        if ( is_null( $l ) ) {
            $l         = new LocoEscapeParser( array( 'U' => 'U', 'a' => 'a', ) );
            self::$lex = $l;
        }
    ...
    public function __construct( $src = '' ) {
        $this->define( '/^(?:\\\\u[0-9A-F]{4,4}|[$_\\pL\\p{Nl}])(?:\\\\u[0-9A-F]{4}|[$_\\pL\\pN\\p{Mn}\\p{Mc}\\p{Pc}])*/ui', array(
            $this,
            'matchWord'
        ) );
        $this->define( '/^\\s+/u', T_WHITESPACE );
        $this->define( '!^//.*!', T_COMMENT );
        $this->define( '!^/\\*.*\\*/!Us', array( $this, 'matchComment' ) );
...
class LocoExtracted implements Countable {
    ...
    public function extractSource( LocoExtractor $ext, $src, $fileref = '' ) {
    ...
    public function pushMeta( $source, $notes = '', $domain = null ) {
        ...
        $entry = array( 'id' => '', 'source' => $source, 'target' => '', 'notes' => $notes, );
        if ( $domain ) {
            $entry['domain'] = $domain;
            $key             = $source . "\1" . $domain;
        } else {
        ...
        return $this->pushMsgid( $key, $entry, $domain );
    }

    public function pushMsgid( $key, array $entry, $domain ) {
    ...
    public function pushMsgidPlural( $key, array $entry ) {
        $key = $key . "\2";
        if ( ! isset( $this->reg[ $key ] ) ) {
            $index               = count( $this->exp );
            $this->reg[ $key ]   = $index;
...
abstract class LocoExtractor {
    ...
    public function rule( $s ) {
        return isset( $this->rules[ $s ] ) ? $this->rules[ $s ] : '';
    }
    ...
    protected function push( LocoExtracted $strings, $rule, array $args, $comment = '', $ref = '' ) {
        ...
        $entry = array( 'id' => '', 'source' => ( $msgid = $key ), 'target' => '', );
        if ( is_int( $c ) && isset( $args[ $c ] ) ) {
            $entry['context'] = $context = $args[ $c ];
            $key              .= "\0" . $context;
        } else if ( ! isset( $msgid{0} ) ) {
            return null;
        } else {
            $context = null;
        }
        ...
        if ( is_int( $d ) && array_key_exists( $d, $args ) ) {
            ...
            $domain = $this->domain or $domain = $strings->getDomain();
        }
        ...
        if ( $comment = $this->comment( $comment ) ) {
            if ( preg_match( '/^xgettext:\\s*([-a-z]+)-format\\s*/mi', $comment, $r, PREG_OFFSET_CAPTURE ) ) {
                ...
                $parse_printf    = ( 'no-' === substr( $entry['format'], 0, 3 ) ) ? false : null;
            }
        ...
        if ( $parse_printf && ( $format = $this->fsniff( $msgid ) ) ) {
            $entry['format'] = $format;
        }
        $index = $strings->pushMsgid( $key, $entry, $domain );
        if ( is_int( $p ) && isset( $args[ $p ] ) ) {
            ...
            if ( false !== $parse_printf && ( $format = $this->fsniff( $msgid_plural ) ) ) {
                $entry['format'] = $format;
            }
            $strings->pushMsgidPlural( $key, $entry );
        }
...
class LocoPHPEscapeParser extends LocoEscapeParser {
    ...
    protected function stripSlashes( $s ) {
        return preg_replace_callback( '/\\\\(x[0-9A-Fa-f]{1,2}|[0-3]?[0-7]{1,2})/', array(
            $this,
            'unescapeMatch'
        ), $s, - 1, $n );
    }
...
function loco_sniff_printf( $s, $p, $limit = 0, $offset = 0 ) {
    $n = 0;
    while ( is_string( $s ) && '' !== $s && false !== ( $i = strpos( $s, '%', $offset ) ) ) {
        if ( 0 !== $i ) {
            $s = substr( $s, $i );
        }
        if ( preg_match( $p, $s, $r ) ) {
            $match = $r[0];
            if ( 0 < $n && isset( $r[1] ) && '' === $r[1] && '%' !== substr( $match, - 1 ) ) {
                return 0;
            }
            if ( ++ $n === $limit ) {
                break;
...
function loco_sniff_php_printf( $s, $limit = 0 ) {
    return loco_sniff_printf( $s, '/^%(?:[1-9]\\d*\\$)?(?:\'.|[-+0 ])*\\d*(?:\\.\\d+)?[suxXbcdeEfFgGo%]/', $limit );
}
...
class LocoPHPExtractor extends LocoExtractor {
    ...
    public function fsniff( $str ) {
        return loco_sniff_php_printf( $str ) ? 'php' : '';
    }

    protected function comment( $comment ) {
        $comment = loco_parse_php_comment( $comment );
        $comment = preg_replace( '/^translators:\\s+/mi', '', $comment );
    ...
    public function extract( LocoExtracted $strings, LocoTokensInterface $tokens, $fileref = '' ) {
        ...
        while ( $tok = $tokens->advance() ) {
            ...
            if ( $depth ) {
                ...
                if ( 0 === $n ) {
                    ...
                    if ( $wp && ( $header = loco_parse_wp_comment( $s ) ) ) {
                        foreach ( $wp as $domain => $tags ) {
                            foreach ( array_intersect_key( $header, $tags ) as $tag => $source ) {
                                $strings->pushMeta( $source, $tags[ $tag ], $domain );
                                $was_header = true;
                            }
                        }
                    }
                ...
                if ( false === stripos( $comment, 'translators:' ) && false === strpos( $comment, 'xgettext:' ) ) {
                    $comment = '';
                }
            }
        }
...
function loco_sniff_js_printf( $s, $limit = 0 ) {
    return loco_sniff_printf( $s, '/^%(?:[1-9]\\d*\\$)?\\+?(?:0|\'[^$])?-?\\d*(?:\\.\\d+)?[b-gijostTuvxX%]/', $limit );
}
...
class LocoJsExtractor extends LocoPHPExtractor {
    ...
    public function fsniff( $str ) {
        return loco_sniff_js_printf( $str ) ? 'javascript' : '';
    }

    public function decapse( $encapsed ) {
...
class LocoTwigExtractor extends LocoPHPExtractor {
    public function tokenize( $src ) {
        $src = '<?php ' . preg_replace( '/\\{#([^#]+)#\\}/su', '/*\\1*/', $src );

        return parent::tokenize( $src );
    }
}

function loco_wp_extractor( $type = 'php' ) {
    static $rules = array(
        '__'              => 'sd',
        '_e'              => 'sd',
        '_c'              => 'sd',
    ...
    if ( 'php' === $type ) {
        $extr = new LocoPHPExtractor( $rules );
    } else if ( 'twig' === $type ) {
        $extr = new LocoTwigExtractor( $rules );
    } else if ( 'js' === $type ) {
        $extr = new LocoJsExtractor( $rules );
    } else {
        throw new InvalidArgumentException( 'No string extractor for ' . $type );
    }
